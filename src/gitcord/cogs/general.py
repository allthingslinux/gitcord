...
from ..constants.messages import (
    ERR_PERMISSION_DENIED, ERR_FILE_NOT_FOUND, ERR_INVALID_YAML, ERR_INVALID_CONFIG,
    ERR_INVALID_CHANNEL_TYPE, ERR_DISCORD_API, ERR_UNEXPECTED, ERR_NO_CONTENT_FOUND, ERR_FETCH_ERROR,
    SUCCESS_CATEGORY_CREATED, SUCCESS_CHANNEL_CREATED, SUCCESS_CATEGORY_UPDATED, SUCCESS_COMMANDS_SYNCED,
    HELP_DOCS, HELP_COMMANDS, HELP_SLASH_COMMANDS, HELP_LINKS, HELP_FOOTER, DELETE_EXTRA_CHANNELS_LABEL
)
from ..constants.permissions import (
    PERM_ADMIN, PERM_MANAGE_CHANNELS, ERR_ADMIN_REQUIRED, ERR_MANAGE_CHANNELS_REQUIRED, ERR_BOT_MISSING_PERMS
)
...
class General(commands.Cog):
    ...
    async def synccommands(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            await interaction.followup.send(
                SUCCESS_COMMANDS_SYNCED.format(count=len(synced)),
                ephemeral=True
            )
            logger.info(
    ...
    async def createcategory_slash(self, interaction: discord.Interaction, yaml_path: Optional[str] = None) -> None:
                ...
                for channel_name in category_config['channels']:
                    try:
                        ...
                        if existing_channel:
                            ...
                            if hasattr(existing_channel, 'position') and 'position' in channel_config and existing_channel.position != channel_config['position']:
                            ...
                            if channel_updated:
                                try:
                                    ...
                                    logger.info("Updated channel '%s' in category '%s'", channel_config['name'], category_config['name'])
                                except Exception as e:
                                    logger.error("Failed to update channel '%s': %s", channel_config['name'], e)
                                    skipped_channels.append(channel_config['name'])
                            else:
                                skipped_channels.append(channel_config['name'])
                                logger.info("Channel '%s' in category '%s' is already up to date", channel_config['name'], category_config['name'])
                        else:
                            ...
                            try:
                                channel_kwargs = {
                                    ...
                                    'topic': channel_config.get('topic', '') if channel_config['type'].lower() == 'text' else None,
                                    'nsfw': channel_config.get('nsfw', False) if channel_config['type'].lower() in ('text', 'voice') else None
                                }
                                ...
                                if channel_config['type'].lower() == 'text':
                                    ...
                                    logger.warning("Skipping channel '%s': Invalid type '%s'", channel_name, channel_config['type'])
                                    skipped_channels.append(channel_config['name'])
                                    continue
                                created_channels.append(new_channel)
                                logger.info("Created new channel '%s' in existing category '%s'", channel_config['name'], category_config['name'])
                            except Exception as e:
    ...
    async def createchannel(self, ctx: commands.Context) -> None:
        ...
        try:
            # Check if the user has permission to manage channels
            if not (isinstance(ctx.author, discord.Member) and ctx.author.guild_permissions.manage_channels):
                embed = create_embed(
                    title="❌ Permission Denied",
                    description=ERR_MANAGE_CHANNELS_REQUIRED,
                    color=discord.Color.red()
        ...
        try:
            ...
            if existing_category:
                ...
                for existing_channel in existing_category.channels:
                    if existing_channel.name not in yaml_channel_names:
                        extra_channels.append(existing_channel)
                        logger.info("Found extra channel '%s' in category '%s' (not in YAML)", existing_channel.name, category_config['name'])
                category_updated = False
                if existing_category.position != category_config.get('position', 0):
                    try:
                        await existing_category.edit(position=category_config.get('position', 0))
                        category_updated = True
                        logger.info("Updated category '%s' position from %d to %d", category_config['name'], existing_category.position, category_config.get('position', 0))
                    except Exception as e:
                        logger.error("Failed to update category position: %s", e)
                for channel_name in category_config['channels']:
                    try:
                ...
                if extra_channels:
                    ...
                    await ctx.send(embed=embed)
                logger.info("Category '%s' processed: %d created, %d updated, %d skipped, %d extra", category_config['name'], len(created_channels), len(updated_channels), len(skipped_channels), len(extra_channels))
                return
            if ctx.guild:
                category_kwargs = {
                    'name': category_config['name'],
                ...
                for channel_name in category_config['channels']:
                    try:
                        ...
                        existing_channel = discord.utils.get(new_category.channels, name=channel_config['name'])
                        if existing_channel:
                            logger.warning("Channel '%s' already exists in category '%s', skipping", channel_config['name'], category_config['name'])
                            continue
                        channel_kwargs = {
                            'name': channel_config['name'],
                            'category': new_category,
                        ...
                        if channel_config['type'].lower() == 'text':
                            ...
                            continue
                        created_channels.append(new_channel)
                        logger.info("Channel '%s' created successfully in category '%s'", channel_config['name'], category_config['name'])
                    except Exception as e:
                        logger.error("Failed to create channel '%s': %s", channel_name, e)
                        continue
                embed = create_embed(
                ...
                if created_channels:
                    ...
                    embed.add_field(name="Created Channels", value=channel_list, inline=False)
                await ctx.send(embed=embed)
                logger.info("Category '%s' with %d channels created successfully by %s", category_config['name'], len(created_channels), ctx.author)
            else:
                embed = create_embed(
                    title="❌ Permission Error",
                    description=ERR_BOT_MISSING_PERMS,

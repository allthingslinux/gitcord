...
class Admin(BaseCog):
    ...
    @commands.has_permissions(administrator=True)
    async def rate_limit_management(self, ctx: commands.Context, action: str = "status", user: discord.Member = None) -> None:
        """
        Manage rate limiting settings and view user stats.
        
        Usage:
        !ratelimit status - Show current rate limit configuration
        !ratelimit stats @user - Show rate limit stats for a specific user
        !ratelimit enable - Enable rate limiting globally
        !ratelimit disable - Disable rate limiting globally
        """
        ...
        if action == "status":
            embed = create_embed(
                ...
                description=f"Current rate limiting settings for this bot"
            )
            embed.add_field(
                name="Status",
                value="🟢 Enabled" if config.rate_limit_enabled else "🔴 Disabled",
                inline=True
            )
            embed.add_field(
                name="Limit",
                value=f"{config.rate_limit_max_commands} commands",
                inline=True
            )
            embed.add_field(
                name="Window",
                value=f"{config.rate_limit_window} seconds",
                inline=True
            )
            embed.add_field(
                name="Exempt Permissions",
                value="• Administrator\n• Manage Guild\n• Manage Channels",
                inline=False
            )
            await ctx.send(embed=embed)
            
        elif action == "stats" and user:
            stats = rate_limiter.get_user_stats(user.id)
            embed = create_embed(
                title=f"📊 Rate Limit Stats for {user.display_name}",
                description=f"Rate limiting statistics for {user.mention}"
            )
            embed.add_field(
                name="Commands Used",
                value=f"{stats['commands_used']}/{stats['max_commands']}",
                inline=True
            )
            embed.add_field(
                name="Status",
                value="🔴 Rate Limited" if stats['is_limited'] else "🟢 Available",
                inline=True
            )
            embed.add_field(
                name="Window",
                value=f"{stats['window_seconds']} seconds",
                inline=True
            )
            ...
            if stats['is_limited']:
                ...
                embed.add_field(
                    name="Reset In",
                    value=f"{reset_in:.1f} seconds" if reset_in > 0 else "Now",
                    inline=False
                )
            ...
            embed = create_success_embed(
                ...
                f"Rate limiting is now enabled with a limit of {config.rate_limit_max_commands} commands per {config.rate_limit_window} seconds."
            )
            await ctx.send(embed=embed)
            ...
            embed = create_success_embed(
                ...
                "Rate limiting has been disabled. Users can now send commands without restrictions."
            )
            await ctx.send(embed=embed)
            ...
            embed = create_error_embed(
                "❌ Invalid Usage",
                "Usage: `!ratelimit <status|stats @user|enable|disable>`\n\n"
                "Examples:\n"
                "• `!ratelimit status` - Show configuration\n"
                "• `!ratelimit stats @username` - Show user stats\n"
                "• `!ratelimit enable` - Enable rate limiting\n"
                "• `!ratelimit disable` - Disable rate limiting"
            )
            await ctx.send(embed=embed)
